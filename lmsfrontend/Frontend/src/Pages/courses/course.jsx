...
export default function CourseManagement() {
  ...
  const deleteCourse = (courseName) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`http://localhost:8006/courses/admin/deletecourse/${courseName}`, fetchOptions)
          .then(response => {
            if (!response.ok) {
  ...
  const selectCourse = async (course) => {
    ...
    try {
      const res = await axios.get(`http://localhost:8006/courses/${course.name}/resources`);
  ...
  const createCourse = async () => {
    try {
      await axios.post("http://localhost:8006/courses/admin/create", { name: newCourse });
      setNewCourse("");
  ...
  const uploadResource = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:8006/courses/admin/${selectedCourse.name}/resource`,
  ...
  const uploadNote = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:8006/courses/admin/${selectedCourse.name}/note`,
        noteData,
            ...
            {resources.length > 0 ? (
              <ul className="resource-list list-disc pl-5">
                {resources.map((res) => (
                  <li key={res}>
                    <a
                      href={`http://localhost:8006/uploads/courses/${selectedCourse.name}/resources/${res}`}
                      target="_blank"
            ...
            {notes.length > 0 ? (
              <ul className="note-list list-disc pl-5">
                {notes.map((note) => (
                  <li key={note}>
                    <a
                      href={`http://localhost:8006/uploads/courses/${selectedCourse.name}/notes/${note}`}
                      download
                    >
                      {note}
                    </a>
