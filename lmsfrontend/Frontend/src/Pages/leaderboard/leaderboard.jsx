...
const Leaderboard = () => {
  ...
  const fetchLeaderboard = async () => {
    try {
      const endpoint = isAdmin
        ? "http://localhost:8006/leaderboard/admin/all"
        : "http://localhost:8006/leaderboard";
  ...
  const handleSearch = async () => {
    try {
      const res = await axios.get(`http://localhost:8006/leaderboard/admin/search/${searchUsername}`);
  ...
  const modifyPoints = async (username, action) => {
    try {
      await axios.post(`http://localhost:8006/leaderboard/admin/${action}point/${username}`);
      fetchLeaderboard();
    } catch (error) {
