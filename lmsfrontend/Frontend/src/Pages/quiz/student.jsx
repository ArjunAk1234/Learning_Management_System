...
function StudentDashboard() {
  ...
  const fetchActiveQuizzes = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:8006/quiz/active');
  ...
  const fetchQuizResults = async (quizId) => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:8006/quiz/results/${studentEmail}/${quizId}`);
  ...
  const checkSubmissionStatus = async (quizId) => {
    try {
      const response = await axios.get(`http://localhost:8006/quiz/check/${quizId}/${studentEmail}`);
...
function QuizTaker({ quiz, studentEmail, onComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await axios.post('http://localhost:8006/quiz/submit', {
        quizId: quiz.id,
