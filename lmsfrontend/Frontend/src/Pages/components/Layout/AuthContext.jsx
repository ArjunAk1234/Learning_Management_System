...
export const AuthProvider = ({ children }) => {
    ...
    const checkUserRole = async (email) => {
        ...
        try {
            ...
            const roleRes = await axios.post("http://localhost:8006/auth/check-role", { email });
    ...
    const fetchProfilePhoto = async (email) => {
        try {
            ...
            if (detailsResponse.data.details && detailsResponse.data.details.photo_path) {
                ...
                const photoUrl = `http://localhost:8006/uploads/${encodeURIComponent(email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg?t=${timestamp}`;
