...
export default function UserProfile() {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      const detailsResponse = await axios.get(`http://localhost:8006/users/details/${encodedEmail}`);
      ...
      if (detailsResponse.data.details) {
        ...
        if (details.photo_path) {
          const photoUrl = `http://localhost:8006/uploads/${encodeURIComponent(user.email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg`;
      ...
      const usernameResponse = await axios.get(`http://localhost:8006/auth/username/email/${encodedEmail}`);
  ...
  const handleCancelEdit = () => {
    ...
    if (userDetails && userDetails.photo_path) {
      setPhotoPreview(`http://localhost:8006/uploads/${encodeURIComponent(user.email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg`);
    } else {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:8006/users/${endpoint}`, formDataToSend, {
        headers: {
      ...
      if (profilePhoto) {
        ...
        const photoUrl = `http://localhost:8006/uploads/${encodeURIComponent(user.email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg?t=${timestamp}`;
        setPhotoPreview(photoUrl);
            ...
            <div className="avatar-container">
              {userDetails && userDetails.photo_path ? (
                <div className="avatar-img">
                  <img
                    src={`http://localhost:8006/uploads/${encodeURIComponent(user.email.replace('@', '_at_').replace('.', '_dot_'))}/photo.jpg?t=${new Date().getTime()}`}
                    alt="Profile"
                  />
                </div>
              ) : (
